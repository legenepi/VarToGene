...
sentinels <- fread("input/Additional_credset_snps_March2025/Locus_PIPSentinel_chr_pos.txt") %>% rename(sentinel = PIP_Sentinel)
...
out_base <- paste0("/scratch/gen1/nnp5/Var_to_Gen_tmp/rare_disease/results_additionalcredsetMarch2025_", DISTANCE/1000, "kb")
...
#save genes only:
fwrite(as.data.frame(results_by_gene$Symbol), "input/Additional_credset_snps_March2025/rare_disease_genes_raw_additionalcredset_March2025.txt", na = "",col.names=F,quote=F)